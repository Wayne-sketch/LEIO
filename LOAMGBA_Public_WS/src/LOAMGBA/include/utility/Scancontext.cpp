...
{
    cout << "utility lib is successfully imported." << endl;
} // coreImportTest
...
//    {
//        pcl::PointXYZ temp;
//        temp.x = _scan_down.points[i].x;
//        temp.y = _scan_down.points[i].y;
//        temp.z = _scan_down.points[i].z + LIDAR_HEIGHT;
...
//    }
    ...
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        ...
        if( azim_range > PC_MAX_RADIUS )
            continue;
        // ---------------------------------------------------
        int row = int(_scan_down[pt_idx].intensity) / 0.25;
        int col;
        if (atan2(_scan_down[pt_idx].y, _scan_down[pt_idx].x) < 0)
        {
            col = int(atan2(_scan_down[pt_idx].y, _scan_down[pt_idx].x) * 180 / M_PI + 360) / 0.5;
        }
        else
        {
            col = int(atan2(_scan_down[pt_idx].y, _scan_down[pt_idx].x) * 180 / M_PI) / 0.5;
        }

        // --------------------------------------------------

        // 行         ring_idx = std::max( std::min( PC_NUM_RING, int(ceil( (azim_range / PC_MAX_RADIUS) * PC_NUM_RING )) ), 1 );
        ring_idx = std::max( std::min( PC_NUM_RING, int(ceil( (azim_range / PC_MAX_RADIUS) * PC_NUM_RING )) ), 1 );
        // 列         sctor_idx = std::max( std::min( PC_NUM_SECTOR, int(ceil( (azim_angle / 360.0) * PC_NUM_SECTOR )) ), 1 );
        sctor_idx = std::max( std::min( PC_NUM_SECTOR, int(ceil( (azim_angle / 360.0) * PC_NUM_SECTOR )) ), 1 );

//        pcl::PointXYZ min;//用于存放三个轴的最小值
//        pcl::PointXYZ max;//用于存放三个轴的最大值
//        pcl::getMinMax3D(points,min,max);
//        int pixel = int(255.0 * (pt.z -min.z) / (max.z-min.z ));
...
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    //cv::Mat dst(20, 60, CV_8UC1);
    //cv::eigen2cv(sc,dst);
    //cv::imwrite("/home/wb/SC_A-LOAM_WS/wb/png/sc.png", dst);
