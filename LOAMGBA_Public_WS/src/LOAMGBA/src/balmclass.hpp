...
public:
  ...
  void calc_eigen()
  {
    Eigen::Matrix3d covMat(Eigen::Matrix3d::Zero());// 滑动窗口的协方差矩阵
    Eigen::Vector3d center(0, 0, 0);// 滑动窗口的中心

    uint asize;
    for(int i=0; i<OCTO_TREE::voxel_windowsize; i++)// 遍历滑动窗口中的当前一个体素
    {
      asize = plvec_tran[i]->size();// 一帧点云个数
      for(uint j=0; j<asize; j++)// 遍历当前帧
      {
          // 计算点pi到滑窗中心的距离矩阵
          // TR(A) = || Pi - P_||²/N
    ...
    feat_eigen_ratio = saes.eigenvalues()[2] / saes.eigenvalues()[ftype];// 特征值
  ...
  void recut(int layer, uint frame_head, pcl::PointCloud<PointType> &pl_feat_map)
  {
    if(octo_state == 0)// 若当前体素没有拟合
    {
      ...
      {
        points_size += plvec_orig[i]->size();// 滑动窗口所有点的数量
      }
      ...
      // 首先，将三维空间划分为范围单位为1米的体素，然后计算体素内协方差矩阵的特征值，以确定体素内的点是否落在同一边缘/平面上，如果是，则保留当前体素，否则将体素划分为8个更小的体素，重复上述操作
      // 滑动窗口所有点的特征值
      calc_eigen(); // calculate eigenvalue ratio
      
      if(isnan(feat_eigen_ratio))// 特征值为空则返回
      {
        feat_eigen_ratio = -1;
        return;
      }
       ...
       */
      // 说得更直白一点，LiDAR BA就是使得对应的边缘点落在同一边缘上，对应的平面点落在同一平面上。因此，作者对多帧之间的相应特征点（边缘/平面点）施加了以下约束：落在同一边缘或平面上。
      ...
      // 到了这还没返回，说明位于前三层，平面或直线拟合成功，建立体素栅格成功
      octo_state = 1;
